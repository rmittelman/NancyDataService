...
namespace NancyDataService
{
    ...
    public static class DataAccess
    {
            ...
            switch (dbFlavor)
            {
                ...
                default:
                    LastError = SerializeDictionary(new Dictionary<string, string>
                                { { "status", "error" }, { "method", "Get_Data" }, { "reason", "dbFlavor missing. Should be Access, Sql or MySql" } });
                    return null;
            }
        }
        ...
        /// <summary>
        /// Verify filter names are valid parameters 
        /// </summary>
        ...
        private static bool Valid_Filters(IDbConnection conn, DbFlavors dbFlavor, DataSourceTypes dataSourceType, string dataSourceName, List<Filter> filters)
        {
            ...
            {
                Dictionary<string, string> parms_or_columns =
                    Get_Parameters_Or_Columns(conn, dbFlavor, dataSourceType, dataSourceName);

                bool isTest = false;
                if (isTest)
                {
                    json = $"{{\"columns\":{JsonConvert.SerializeObject(parms_or_columns)}}}";
                    json = InsertJsonProperties(json, Formatting.None, "status", "testing");
                    LastError = json;
                    return false;
                }
                ...
                foreach (var filter in filters)
                {
                    if (!parms_or_columns.ContainsKey(filter.Field))
                    {
                        string itemName = dataSourceType == DataSourceTypes.procedure ? "parameter" : "column";
                        string msg = $"{dataSourceType.ToString()} '{dataSourceName}' does not have a {itemName} named '{filter.Field}'.";
                        LastError = SerializeDictionary(new Dictionary<string, string> { { "status", "error" }, { "reason", msg } });
                        return false;
                    }
                }
                return true;
        ...
        /// <param name="dbFlavor">A <see cref="DbFlavors"/> enumeration member. Ex: Access/Sql/MySql.</param>
        /// <param name="dataSourceType">A <see cref="DataSourceTypes"/> enumeration member. Ex: table/procedure.</param>
        /// <param name="dataSourceName">SqlQuery / StoredProcedure The table to select the data from.</param>
