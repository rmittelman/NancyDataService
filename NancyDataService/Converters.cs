...
namespace Converters
{
        ...
        public override object ReadJson(Newtonsoft.Json.JsonReader reader, System.Type objectType, object existingValue, Newtonsoft.Json.JsonSerializer serializer)
        {
            ...
            if (jObject["Rows"] != null)
            {
                foreach (JArray jRow in jObject["Rows"])
                {
                    ...
                    var rowObject = jRow.ToObject<System.Object[]>();
                    row.ItemArray = rowObject;

                    // fix "" columns that should be nulls
                    for (int i = 0; i < rowObject.Length; i++)
                    {
                        if (rowObject[i] == null && row[i].ToString() == string.Empty && table.Columns[i].AllowDBNull)
                            row[i] = DBNull.Value;
                    }
                    table.Rows.Add(row);
                }
            ...
            // Add the primary key column(s) to the table if needed
    ...
    //            {
    //                var myItem = new NancyDataService.DataItem()
    //                {
    ...
    //                //writer.WritePropertyName(column.DataType);

    //                JsonSerializer.Serialize(writer, myItem, options);
    //            }
    //            writer.WriteEndObject();
    //        }
    ...
    //    //private string Simple_Data_Type(DataColumn col)
    //    //        {
